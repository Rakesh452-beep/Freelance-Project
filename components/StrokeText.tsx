...
const StrokeText = ({
  ...
  useEffect(() => {
    ...
    const staggerConfig: gsap.TweenVars["stagger"] = reverse ? { each: stagger, from: "end" } : stagger;
    ...
    let scrollTrigger: gsap.plugins.ScrollTrigger | null = null;
  ...
  const viewBox = box
    ...
    : `0 ${-fontSize} 600 ${fontSize * 1.3 * lines.length}`;

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === "hover" ? "stroke-text--hover" : ""} ${className}`.trim()}
      style={{ ...style, "--stroke-text-height": `${Math.round(fontSize * 1.3 * lines.length)}px` } as React.CSSProperties}
      role="img"
    ...
    >
      <svg
        ...
        aria-hidden="true"
        style={box ? { height: "auto", aspectRatio: `${box.width} / ${box.height}` } : undefined}
      >
        {fillMode === "wipe" && box && (
          <defs>
            ...
            <Fragment key={`s-${li}`}>
              {Array.from(line).map((char, ci) => (
                <tspan
                  ...
                  key={`s-${li}-${ci}`}
                  x={ci === 0 ? "0" : undefined}
                  dy={ci === 0 && li > 0 ? lineHeight : undefined}
                >
                  {char}
                </tspan>
              ))}
